			...
			if shouldProcessEvent {
				ctxlog.WithEvent(o, "Predicates").DebugJSON(ctx,
					...
					ctxlog.ReconcileEventsFromSource{
						...
						Type:                     "predicate",
						Message: fmt.Sprintf("For type %s, filter based on labels with key %s and value %s",
							bdv1.SecretType, bdv1.LabelDeploymentSecretType, o.GetLabels()[bdv1.LabelDeploymentSecretType]),
					},
				)
			}
