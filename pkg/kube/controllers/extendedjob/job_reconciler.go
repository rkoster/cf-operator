...
func (r *ReconcileJob) persistOutput(ctx context.Context, instance *batchv1.Job, ejob ejv1.ExtendedJob) error {
	...
	for _, c := range pod.Spec.Containers {
		...
		if err != nil {
			ctxlog.WithEvent(&ejob, "ExtendedJob").Errorf(ctx, "non-JSON output was emitted for container '%s', secret '%s' cannot be created", instance.GetName(), secretName)
			return errors.Wrap(err, "invalid output format")
		}

		// Put full job output in debug log
		ctxlog.Debugf(ctx, "Printing logs from container %s: %s", c.Name, string(result))
